<main id="main" class="main">
    ...
    <?php if (!empty($filteredMonth)): ?>
        <h3>Filtered Month: <?= esc($filteredMonth); ?></h3>
    <?php endif; ?>
    ...
    <div class="container mt-4">
        <p>Note: After selecting the month, submit Recovered and Disposed Materials one by one. <br> 
            Ensure to submit all of them in one session, then click 'Finish Monthly Submission' to complete the process successfully for the month.
        </p>
    </div>
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="mt-4">
                <div class="card-body">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <?php foreach ($recycledSummary as $items): ?>
                                <tr>
                                    <td><?= esc($items['batch_id']) ?></td>
                                    <td><?= esc($items['Material_Type']) ?></td>
                                    <td><?= esc($items['Material']) ?></td>
                                    <td><?= esc($items['total_mass']) ?></td>
                                    <td><?= esc(date("F Y", strtotime($items['recycled_at']))) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
